...
wss.on('connection', (ws, req) => {
  ...
  const userId = url.searchParams.get('userId') || 'u-' + Math.random().toString(36).slice(2, 8)
  const name = url.searchParams.get('name') || 'You'

  clients.set(ws, { userId, name, presence: { state: 'online', activeRoom: null } })
  ...
  ws.on('close', () => {
    const meta = clients.get(ws)
    clients.delete(ws)
    console.log(`[-] disconnected: ${meta ? meta.name : '?'} — clients: ${clients.size}`)
  })
...
server.listen(PORT, () => {
  ...
  console.log(`bot: ${BOT_ENABLED ? `every ${BOT_INTERVAL_MS}ms` : 'off'} (set BOT=off to disable)`)
})
